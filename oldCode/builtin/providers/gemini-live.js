    ...
    return new Promise((resolve, reject) => {
      ...
      ws.on('message', (data) => {
        try {
          ...
          if (parsed.error) {
            console.error('[Gemini Live] Server error payload received:', parsed.error);
            ws.close();
            reject(new Error(`Gemini Live Error (${parsed.error.code || 'API'}): ${parsed.error.message || JSON.stringify(parsed.error)}`));
            return;
          }
          ...
          if (parsed.serverContent?.modelTurn?.parts) {
            for (const part of parsed.serverContent.modelTurn.parts) {
              if (part.thought) {
                ...
                if (thoughtText) callbacks.onThoughtChunk?.(thoughtText);
              } else if (part.text) {
